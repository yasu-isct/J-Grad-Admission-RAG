...
It is generated by `tests/run_rule04e_browser_audit.py` in real Chrome and is bound to the audited
`app.js` SHA-256, so CI rejects stale records after renderer changes.
`rule04f_browser_acceptance_v1.json` records the information-engineering B-schedule match, missing
route, A-schedule, and wrong-parent paths at both widths. It is generated by
`tests/run_rule04f_browser_audit.py` in real Chrome and binds p.52 plus the current `app.js` hash.
