...
def test_real_pdf_kb_registers_as_ready_corpus_entry(
    ...
    assert entry.index_manifest.payload_count == entry.index_manifest.vector_count == 318
...
def test_real_pdf_entry_is_unchanged_when_adding_second_institution(
    ...
    assert len(real_document_kb.facts) == len(real_document_kb.retrieval_units) == 318
...
def test_real_pdf_identity_is_selected_only_by_reviewed_active_policy(
    ...
    assert selected.entry.index_manifest.payload_count == 318
    assert len(real_document_kb.facts) == len(real_document_kb.retrieval_units) == 318
...
def test_real_pdf_global_corpus_search_preserves_document_qualified_pages(
    ...
    assert real_entry.index_manifest.payload_count == 318
...
def test_real_pdf_reviewed_report_plan_resolves_exact_current_fact(
    ...
    assert len(real_document_kb.facts) == 318
...
def test_real_pdf_derives_traceable_index_payload_shape(
    ...
    assert len(payloads) == expected_count == 318
...
def test_real_pdf_embedding_text_v1_is_complete_and_structure_preserving(
    ...
    assert len(facts) == len(units) == expected["retrieval_unit_count"] == 318
...
def test_real_pdf_fake_embeddings_are_deterministic_without_mutating_kb(
    ...
    assert len(first) == real_pdf_manifest["expected"]["retrieval_unit_count"] == 318
...
def test_real_pdf_local_index_is_aligned_normalized_and_byte_deterministic(
    ...
    assert first_manifest.payload_count == first_manifest.vector_count == 318
    ...
    assert mapped.vectors.shape == (318, 8)
    ...
    assert [payload.row_index for payload in mapped.payloads] == list(range(318))
...
def test_real_pdf_build_index_cli_reports_frozen_fake_artifacts(
    ...
    assert summary["payload_count"] == summary["vector_count"] == 318
    ...
    assert loaded.manifest.payloads_sha256 == (
        "a5f2c386572ef09a4a4db129809d3451ceb4ac44e0010ddbca19adbf1d83fcff"
    )
    assert loaded.manifest.vectors_sha256 == (
        "50adbe6ba7fb51072bb9343325f6dcaf87462468a83a67500e1d1ae43195909c"
    )
...
def test_real_pdf_vector_search_matches_independent_numpy_ranking_and_cli(
    ...
    assert result.manifest.payload_count == result.manifest.vector_count == 318
    assert (
    ...
    assert [hit.row_index for hit in result.hits] == [228, 317, 55, 273, 227]
    assert [hit.score for hit in result.hits] == pytest.approx(
        [
            0.9264391660690308,
            0.8517074584960938,
...
def test_real_pdf_fake_hybrid_plumbing_is_stable_and_cli_equivalent(
    ...
    assert characterization_sha256 == (
        "491221cc3f2d86539a0e38e24e586776b0b9c221a25435c44d9eedcd3049e6c0"
    )
    assert aggregate_sha256 == ("806733c665d46c02c4c9b322928de5824cdcbaadc8fa80d0ae55193243dd9f45")
...
def test_real_pdf_metadata_inventory_and_hard_filter_examples(
    ...
    assert scope_type_counts == {"department": 121, "global": 55, "unknown": 142}
    ...
    examples = (
        ...
        (MetadataFilter(scope_types=("department",)), 121),
        (MetadataFilter(scope_targets=("情報工学系",)), 13),
...
def test_real_pdf_metadata_no_filter_and_scope_sensitive_characterization(
    ...
    assert len(full_preference.hits) == 318
    assert full_preference.eligible_row_count == 318
    both = next(
        hit
    ...
    assert outcome_sha256 == ("b71cbf7af7dc3ff30c1b8f808309eb3ed797a6eb3f79a48541a991a3a8492776")
...
def test_real_pdf_builds_34_canonical_evidence_packs_with_official_evidence(
    ...
    assert aggregate_sha256 == "2bfde928c6055cc799463a36154217db0db6b4c39748de62124728859e7f31ae"
...
def test_real_pdf_fake_retrieval_evaluation_is_deterministic_and_independently_scored(
    ...
    assert hashlib.sha256(canonical).hexdigest() == (
        "bffe02dab4622d049cc5d348d56088f5e7d2392fabd3dfd4af13d4060aa5b948"
    )
